...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
    ...
    Relational Databases are stores of data and SQL is the language used to query that data.

- [ ] Why do tables need a `primary key`?

    All tables need a primary key so that the data within each row has a reference.

- [ ] What is the name given to a table column that references the primary key on another table.

    A column that references the primary key on another table is a foreign key.

- [ ] What do we need in order to have a _many to many_ relationship between two tables.

    We need foreign key and connection tables in order to have a many to many relationship between to tables.
